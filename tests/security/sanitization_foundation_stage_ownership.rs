...
fn only_policy_stage_owns_final_action_selection() {
    ...
    assert!(boundaries
        .iter()
        .any(|boundary| boundary.stage_id == StageId::PolicyResolution
            && boundary.owns_policy_decision));
...
fn only_transform_stage_may_mutate_payloads() {
    ...
    assert!(boundaries.iter().any(
        |boundary| boundary.stage_id == StageId::Transformation && boundary.may_mutate_payload
    ));
}
