...
#[tokio::test]
async fn hardening_flow_covers_rate_limit_and_metrics_in_single_runtime() {
    ...
    .await;
    let api_key_path = write_key_file("e2e-hardening-key");
    let provider_key_path = write_key_file("e2e-provider-key");
...
"#,
    ...
    let body = serde_json::json!({
        "model": "gpt-4o-mini",
        "messages": [{"role":"user","content":"hello hardening"}]
    });

    let first = client
        .post(format!("{}/v1/chat/completions", runtime.base_url()))
        .bearer_auth("e2e-hardening-key")
        .json(&body)
        .send()
        .await
    ...
    assert!(metrics.contains("pokrov_requests_total"));
