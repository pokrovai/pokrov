...
#[tokio::test]
async fn responses_passthrough_accepts_single_bearer_for_codex_compatibility() {
    ...
    let gateway_key_path = write_key_file("gateway-and-upstream-key");
...
"#,
    ...
    let response = client
        ...
        .header("authorization", "Bearer gateway-and-upstream-key")
    ...
    assert_eq!(forwarded_auth[0].as_deref(), Some("Bearer gateway-and-upstream-key"));
...
#[tokio::test]
async fn responses_passthrough_blocks_single_bearer_without_explicit_opt_in() {
    let provider = start_mock_provider(MockProviderMode::Json {
        status: 200,
        body: serde_json::json!({
            "id": "chatcmpl-1",
